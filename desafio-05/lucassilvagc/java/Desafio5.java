...
public class Desafio5 {
  ...
  private static void areaEmployees() {
    ...
    for (Area area : areas) {
      areaMap.put(
          area.getNome(),
          funcionarios.stream().filter(c -> c.getArea().equals(area.getCodigo())).count());
    }
    ...
    for (int i = 0; i < areaMap.size(); i++) {
      if (areaMap.values().toArray()[i].equals(maxValue)) {
        System.out.println(
            "most_employees|"
                + areaMap.keySet().toArray()[i]
                + "|"
                + areaMap.values().toArray()[i]);
  ...
  private static void lastNameSalary() {
    ...
    funcionarios.stream()
        .collect(Collectors.groupingByConcurrent(Funcionario::getSobrenome, Collectors.counting()))
        .forEach(
            (surname, count) -> {
              if (count > 1) surnames.add(surname);
            });
    for (String surname : surnames) {
      Double maxSalary =
          funcionarios.stream()
              .filter(f -> f.getSobrenome().equals(surname))
              .max(Comparator.comparingDouble(Funcionario::getSalario))
              .get()
              .getSalario();
      funcionarios.stream()
          .filter(f -> f.getSobrenome().equals(surname))
          .filter(f -> f.getSalario().equals(maxSalary))
          .forEachOrdered(
              f ->
                  System.out.println(
                      "last_name_max|"
