...
unsigned int create_hash(const char *word, unsigned int buckets)
{
    ...
    {
        hash = (hash * 31) + (unsigned int)(toupper(word[i]));
    }
...
char *tokenize(char *expression)
{
    ...
    for (unsigned int c = 0, shift = 0; c <= len; c++)
    {
        ...
        token[shift++] = (char)toupper(expression[c]);
    }
